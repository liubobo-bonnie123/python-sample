...
def makeXMLfriendly(ugly_string):
  ...
  Returns an XML-friendly ASCII version of the inputted string. Special characters
  are replaced by their XML character references."""
  assert(type(ugly_string) == str or type(ugly_string) == unicode)
  return ugly_string.encode('ascii','xmlcharrefreplace')

def convertentities(ugly_string):
  """ convertentities : string -> string
  Forces utf8 special characters into XML character references."""
  assert(type(ugly_string) == str or type(ugly_string) == unicode)
  return ugly_string.decode('utf-8').encode('ascii','xmlcharrefreplace')
...
def makecleanelement(open, ugly_string, close):
  ...
  open and close."""
  assert(type(ugly_string) == str or type(ugly_string) == unicode)
  description_soup = BeautifulSoup(convertentities(ugly_string))
